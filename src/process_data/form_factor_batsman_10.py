...
form_file = pd.read_csv('../Processed/form_index_batsman_10.csv')
...
random = [213080,217481,217811,217978,223334,223902,226352,226375
...
for match_idx in range(len(match_ids)):
    ...
    if match_id in form_file['match_id'].tolist() :
        continue
    ...
    for team in ['T1','T2']:
        ...
        for idx,players in enumerate(team_lineup['Players']):
            ...
            p['bat_avg_sr'] = p['bat_avg'] * p['bat_SR']
            bat_avg_sr =  p['bat_avg_sr'].iloc[len(p)-1]
...
#            p_filter['Runs1']= p_filter['Runs'].astype(str).map(lambda x : re.sub("[^0-9a-z]","",x)).astype(int)
            ...
            if len(p_filter) > 10:
                p_filter = p_filter[len(p_filter)-10:]            
            ...
            try:
                ...
                if sd==0:
                    form_index = rec_avg_sr/bat_avg_sr                
                else:
    ...
    if((match_idx+1)%10==0):
        ...
        model_df.to_csv(r'../Processed/form_index_batsman_10+v2.csv',index= False)   
    model_df.to_csv(r'../Processed/form_index_batsman_10+v2.csv',index= False)   
